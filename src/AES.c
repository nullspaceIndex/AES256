...
Matrix4x4 AddRoundKey(Matrix4x4 state, Matrix4x4 roundKey) {
    ...
    for (int col = 0; col < 4; col++) {
        for (int row = 0; row < 4; row++) {
            result.data[col][row] = state.data[col][row] ^ roundKey.data[col][row];
        }
    }
    return result;
}
...
Matrix4x4 SubBytes(Matrix4x4 state) {
    ...
    for (int col = 0; col < 4; col++) {
        for (int row = 0; row < 4; row++) {
            uint8_t byte = state.data[col][row];
            result.data[col][row] = sbox[byte];
        }
    }
    return result;
}
...
Matrix4x4 ShiftRows(Matrix4x4 state) {
    ...
    for (int row = 0; row < 4; row++) {
        for (int col = 0; col < 4; col++) {
            ...
            result.data[col][row] = state.data[(col + row) % 4][row];
        }
    }
